...
class Programa_didacticoDAO {
    ...
    public function findAll_by_estado($estado) {
        $this->conexion->conectar();
        $query = "SELECT * FROM programa_didactico pd JOIN programa_extenso pe ON pd.pe_id = pe.pe_id JOIN asignatura a ON pe.asig_codigo = a.asig_codigo JOIN usuario u ON pe.usu_rut = u.usu_rut WHERE pd.pd_borrador = " . $estado . " ";
        $result = $this->conexion->ejecutar($query);
        $i = 0;
        $programa_didacticos = array();
        while ($fila = $result->fetch_row()) {
            ...
            $autor = utf8_decode($fila[43]) . " " . utf8_decode($fila[44]);
    ...
    public function find_aprobados_By_m_id_and_periodo($m_id, $m_fechaInicio, $m_fechaFin) {
        $this->conexion->conectar();
        $query = "SELECT * "
                . " FROM programa_didactico pd "
                . " JOIN programa_extenso pe ON pd.pe_id = pe.pe_id "
                . " JOIN asignatura a ON pe.asig_codigo = a.asig_codigo "
                . " JOIN usuario u ON pe.usu_rut = u.usu_rut "
                . " WHERE a.m_id = '$m_id' AND pd.pd_borrador = 0 AND pd.pd_fecha_modificacion >= '$m_fechaInicio' AND pd.pd_fecha_modificacion <= '$m_fechaFin' ";
        $result = $this->conexion->ejecutar($query);
        $i = 0;
        $programa_didacticos = array();
        while ($fila = $result->fetch_row()) {
    ...
    public function findPorVencer_by_periodo($fechaInicio, $fechaFin) {
        ...
        $query = "SELECT * "
                ...
                . " JOIN asignatura a ON pe.asig_codigo = a.asig_codigo "
                . " JOIN usuario u ON pe.usu_rut = u.usu_rut "
                . " WHERE pd.pd_borrador = 0 AND pe.pe_fecha_fin >= '$fechaInicio' AND pe.pe_fecha_fin <= '$fechaFin'; ";
        $result = $this->conexion->ejecutar($query);
        $i = 0;
        $programa_didacticos = array();
        while ($fila = $result->fetch_row()) {
